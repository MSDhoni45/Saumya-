...
import { useConversationMessages, useSendMessage, useUpdateConversation } from "@/lib/inbox/queries";
...
export function MessageThread({
  ...
  if (!conversation) {
    return (
      <div className={`flex-1 flex-col items-center justify-center gap-1 bg-slate-50 px-6 text-center ${className ?? "hidden md:flex"}`}>
        <p className="text-sm font-medium text-slate-600">Select a conversation</p>
        <p className="text-sm text-slate-400">Choose a conversation from the list to view its message history.</p>
      </div>
    );
  }

  return <ActiveThread businessId={businessId} conversation={conversation} currentUser={currentUser} onBack={onBack} className={className} />;
}
...
function ActiveThread({
  ...
  // Switching threads should always start pinned to the bottom — otherwise a
  // long scroll position carried over from the previous conversation would
  // suppress the initial scroll-to-latest.
  useEffect(() => {
  ...
  const canTakeOver = conversation.status !== "closed" && !isAssignedToMe;
  const canHandBack = isAssignedToMe;

  const handleTakeOver = () => {
    updateConversation.mutate({
      conversationId: conversation.id,
      payload: { status: "handoff", assigned_user_id: currentUser.id },
    });
  };
  ...
  const handleHandBack = () => {
    updateConversation.mutate({
      conversationId: conversation.id,
      payload: { status: "open", assigned_user_id: null },
    });
  };
  ...
  return (
    <div className={`min-w-0 flex-1 flex-col ${className ?? "flex"}`}>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {canTakeOver && (
            <button
              ...
              className="rounded-md border border-brand-200 bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-700 transition hover:bg-brand-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Take over
            </button>
          )}
          {canHandBack && (
            <button
              ...
              disabled={updateConversation.isPending}
              className="rounded-md border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Hand back to AI
            </button>
          )}
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50 px-4 py-4">
          ...
          groupByDay(messages).map(([day, dayMessages]) => (
            <div key={day} className="space-y-3">
              <div className="flex justify-center">
                <span className="rounded-full bg-white px-3 py-1 text-xs font-medium text-slate-500 shadow-sm">{day}</span>
              </div>
      ...
      <MessageComposer
        ...
        onSend={(text) => sendMessage.mutate({ conversation, text })}
      />
    </div>
  );
}
